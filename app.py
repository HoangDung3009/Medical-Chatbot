...
nlp_model = NeuralNetwork(input_size, hidden_size, output_size).to(device)
...
def index():
    data = []
    user_symptoms.clear()
    file = open("static/assets/symptoms.txt", "r")
...
def predict_symptom():
    ...
    if sentence.replace(".", "").replace("!", "").lower().strip() == "done":
        ...
        if not user_symptoms:
            response_sentence = random.choice(
                ["I can't know what disease you may have if you don't enter any symptoms :)",
                 "Ceri can't know the disease if there are no symptoms...",
            ...
            description = \
            diseases_description.loc[diseases_description['Disease'] == disease.strip(" ").lower(), 'Description'].iloc[
                0]
            precaution = disease_precaution[disease_precaution['Disease'] == disease.strip(" ").lower()]
            precautions = 'Precautions: ' + precaution.Precaution_1.iloc[0] + ", " + precaution.Precaution_2.iloc[
                0] + ", " + precaution.Precaution_3.iloc[0] + ", " + precaution.Precaution_4.iloc[0]
            ...
            if np.mean(severity) > 4 or np.max(severity) > 5:
                response_sentence = response_sentence + "<br><br>Considering your symptoms are severe, and Ceri " \
                                                        "isn't a real doctor, you should consider talking to one. :) "
            user_symptoms.clear()
            severity.clear()
        ...
        if symptom == "greeting":
            response_sentence = "Hello"
        elif prob > .5:
            responses = [f"Hmm, I'm {(prob * 100):.2f}% sure this is " + symptom + ".",
                 f"Perhaps you are {(prob * 100):.2f}% " +symptom + ".",
                 f"I guess you are {(prob * 100):.2f}% " +symptom + " right now."]
            response_sentence = random.choice(responses)
